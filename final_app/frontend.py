...
# Absolute paths anchored to the project root so file I/O works regardless
# of which directory streamlit is launched from.
# This file lives at: <project_root>/final_app/frontend.py
#   .parent   → final_app/
#   .parent.parent → <project_root>
_PROJECT_ROOT = Path(__file__).resolve().parent.parent
_OUTPUTS_DIR = _PROJECT_ROOT / "outputs"   # where generated .md blogs are saved
_IMAGES_DIR = _PROJECT_ROOT / "images"     # where generated images are saved
...
def _resolve_image_path(src: str) -> Path:
    # Resolve relative image paths (e.g. "images/foo.png") against the project
    # root, not CWD — the app may be launched from final_app/ or anywhere else.
    src = src.strip().lstrip("./")
    return (_PROJECT_ROOT / src).resolve()
...
def list_past_blogs() -> List[Path]:
    """
    Returns .md files from the outputs/ directory, newest first.
    Uses the absolute _OUTPUTS_DIR path so it works regardless of CWD.
    """
    if not _OUTPUTS_DIR.exists():
        return []
    files = [p for p in _OUTPUTS_DIR.glob("*.md") if p.is_file()]
...
with st.sidebar:
    ...
    if not past_files:
        st.caption("No saved blogs found in outputs/.")
...
if out:
    ...
    with tab_preview:
        ...
        if not final_md:
            ...
            bundle = bundle_zip(final_md, md_filename, _IMAGES_DIR)
            st.download_button(
    ...
    with tab_images:
        ...
        images_dir = _IMAGES_DIR   # absolute path to project_root/images/
